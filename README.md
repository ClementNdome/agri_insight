...
[Website](https://agri-insight.spationex.com/landing/)

A comprehensive GIS-based platform empowering farmers with real-time, data-driven insights into agricultural operations. Users can define farm areas on an interactive map, analyze crop health using satellite-derived vegetation indices from Google Earth Engine, and receive AI-powered recommendations for yield optimization, irrigation, and sustainable practices.

## Features

- **Interactive Map Interface**: Draw and manage farm plots (areas of interest) with geospatial precision using Leaflet maps.
- **Vegetation Index Analysis**: Supports multiple vegetation indices (NDVI, EVI, SAVI, NDMI, NBR, NDWI, GNDVI, OSAVI) for tailored crop monitoring.
- **Google Earth Engine Integration**: Fetches high-resolution satellite data directly via the GEEMAP API.
- **Real-Time Monitoring and AI Insights**: Provides automated alerts, yield predictions, and personalized recommendations using machine learning.
- **Data Visualization**: Offers interactive charts, dashboards, and exportable reports for historical trends and analytics.
- **RESTful API**: Secure and scalable API for data access, third-party integrations, and mobile apps.
- **PostGIS Database**: Robust spatial database for geographic queries and data storage.
- **Mobile Responsiveness**: Optimized for use on smartphones and tablets in the field.
...
- **Frontend**: HTML5, JavaScript, Leaflet.js, and Bootstrap 5 for responsive UI.
- **Satellite Data**: Google Earth Engine API for remote sensing.
- **Spatial Processing**: GEEMAP, GDAL, GEOS, GeoPandas, and Rasterio.
...
- **Visualization**: Matplotlib/Plotly for charts and Folium for embedded maps.
...
Thank you for using AgriInsight! For collaborations, monetization inquiries, or custom integrations, contact [mwakaviclement@gmail.com](mailto:mwakaviclement@gmail.com).